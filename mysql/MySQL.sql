...
ALTER TABLE 고객 DROP COLUMN 가입날짜;
	...
	[예제 7-7] ----> MySQL에서는 CHECK 제약 조건은 지원하지 않음(실행은 되지만 제약 조건이 추가되지 않음)
ALTER TABLE 고객 ADD CONSTRAINT CHK_AGE CHECK(나이 >= 20); 


	[예제 7-8] ----> MySQL에서는 CHECK 제약 조건은 지원하지 않음(실행 안됨)
...
FROM     제품;
	...
	[예제 7-17]----> MySQL에서는 두 문장 모두 실행 가능
...
FROM    제품;
	...
	[예제 7-29]----> MySQL에서는 두 문장 모두 실행 가능
...
WITH CHECK OPTION;
	...
	[예제 7-56]----> MySQL에서는 실행 안됨(업체별제품수처럼 변경이 불가능한 뷰를 생성할 때는 어차피 삽입, 수정, 삭제 연산이 불가능하기 때문에 MySQL에서는 with check option을 붙이지 않음)
...
(변경)----> 따라서 다음과 같이 with check option 없이 작성해야 함
